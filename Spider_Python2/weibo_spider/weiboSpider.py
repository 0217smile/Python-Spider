...
class weibo:
    cookie = {"Cookie": "_T_WM=d167c25ae881f83dc5fa3ee5b4943560; SCF=AnVd9U7emcP89xoOzY1rozGrlbu8FUTqhzo3s2nmdyMMHX413RZzyquXBEek-pL59iO2jZ9WNcFuVigZ19KQRGE.; _T_WL=1; _WEIBO_UID=5839578868; H5_INDEX=0_all; H5_INDEX_TITLE=%E5%90%AC%E6%AD%8C%E6%8A%8A%E5%A6%B9%E5%B0%8F%E6%98%9F%E6%98%9F; SUB=_2A250Q7c2DeThGeNG6FsU9ybEzTSIHXVXz9l-rDV6PUJbkdAKLRTWkW0h53EHa747tfBomyKQ-U3Z_wBUAA..; SUHB=0btsYOQNuF2Dmn; SSOLoginState=1497876325; M_WEIBOCN_PARAMS=featurecode%3D20000320%26luicode%3D10000011%26lfid%3D2302575839578868%26uicode%3D20000174"}  # 将your cookie替换成自己的cookie
    ...
    def getUserName(self):
        try:
            url = 'http://weibo_spider.cn/%d/info' % (self.user_id)
            response = requests.get(url, cookies=weibo.cookie)
    ...
    def getUserInfo(self):
        try:
            url = 'http://weibo_spider.cn/u/%d?filter=%d&page=1' % (self.user_id, self.filter)
    ...
    def getWeiboInfo(self):
        try:
            ...
            for page in range(1, pageNum + 1):
                url2 = 'http://weibo_spider.cn/u/%d?filter=%d&page=%d' % (self.user_id, self.filter, page)
    ...
    def writeTxt(self):
        try:
            ...
            for i in range(1, self.weiboNum2 + 1):
                ...
                result = result + text
            if os.path.isdir('weibo_spider') == False:
                os.mkdir('weibo_spider')
            f = open("weibo_spider/%s.txt" % self.user_id, "wb")
            f.write(result)
            f.close()
...
user_id = 6083626014  # 可以改成任意合法的用户id（爬虫的微博id除外）
